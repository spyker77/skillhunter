...
async def scan_single_search_page(job_title, page_num, session):
    ...
    for _ in range(10):
        try:
            async with session.get("https://www.simplyhired.com/search", params=payload) as resp:
                try:
            ...
            logger.error("🚨 ClientConnectorError occurred while scanning indeed.com.")
            await asyncio.sleep(60)
        except ServerDisconnectedError:
            logger.error("🚨 ServerDisconnectedError occurred while scanning indeed.com.")
            await asyncio.sleep(60)
...
async def main(job_title, sh_links_we_already_have, skills):
    ...
    async with aiohttp.ClientSession(headers={"user-agent": fake_agent, "Connection": "close"}) as session:
        ...
        for _ in range(10):
            try:
                vacancies_without_skills = await fetch_all_vacancy_pages(all_links, sh_links_we_already_have, session)
